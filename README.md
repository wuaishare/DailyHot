...
展示版本来自 `package.json`，格式为 `v1.4.4-alpha (构建号)`；括号内构建号优先由 Git 提交序号自动生成。CI 若是浅克隆会在构建时自动补全历史再计算提交序号；如果补全失败，会退回到当前提交 SHA 派生数字，仍保证每次提交部署后缓存版本变化。前端缓存刷新只依据构建号判断，正常提交部署后无需手动修改缓存版本。
