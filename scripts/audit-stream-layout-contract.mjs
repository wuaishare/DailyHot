...
assert.match(template, /v-if="usesPreviewCoverProfile && !minimalMode && showImages && entry\.cover"[\s\S]{0,160}category-stream__media is-cover is-previewable/);
...
assert.match(component, /SQUARE_COVER_SOURCES = new Set\(\["xiaohongshu"\]\)/);
assert.match(component, /LANDSCAPE_COVER_SOURCES = new Set\(\["baidu"\]\)/);
assert.match(component, /is-square-cover-source-page[\s\S]{0,260}grid-template-columns: 42px 84px minmax\(0, 1fr\)/);
assert.match(component, /category-stream\.is-source-page \.category-stream__media \{[\s\S]{0,160}width: 112px;[\s\S]{0,80}height: 84px;[\s\S]{0,100}place-items: center end;/);
...
assert.match(component, /is-square-cover-source-page \.category-stream__media\.is-cover img,[\s\S]{0,120}is-landscape-cover-source-page \.category-stream__media\.is-cover img,[\s\S]{0,180}is-fixed-portrait-cover-source-page \.category-stream__media\.is-cover img[\s\S]{0,240}object-fit: cover;/);
assert.match(component, /FIXED_PORTRAIT_COVER_SOURCES = new Set\(\["douban-movie", "weread"\]\)/);
assert.match(component, /is-fixed-portrait-cover-source-page[\s\S]{0,260}grid-template-columns: 42px 58px minmax\(0, 1fr\)/);
assert.doesNotMatch(component, /is-fixed-portrait-cover-source-page \.category-stream__media\.is-cover img[\s\S]{0,100}object-fit: contain;/);
assert.match(template, /<n-image[\s\S]{0,420}:preview-src="coverSrc\(entry\.cover\)"[\s\S]{0,420}tabindex: 0[\s\S]{0,160}role: 'button'/);
assert.match(component, /handleCoverPreviewKeydown[\s\S]{0,180}currentTarget\?\.click/);
assert.match(component, /@media \(max-width: 680px\)[\s\S]*grid-template-columns: 34px 76px minmax\(0, 1fr\)[\s\S]*width: 76px;[\s\S]*height: 64px/);
assert.match(component, /@media \(max-width: 680px\)[\s\S]*is-square-cover-source-page[\s\S]*grid-template-columns: 34px 64px minmax\(0, 1fr\)/);
assert.match(component, /@media \(max-width: 680px\)[\s\S]*is-landscape-cover-source-page \.category-stream__media[\s\S]{0,100}height: 57px;/);
assert.match(component, /@media \(max-width: 680px\)[\s\S]*is-fixed-portrait-cover-source-page[\s\S]*grid-template-columns: 34px 44px minmax\(0, 1fr\)/);
