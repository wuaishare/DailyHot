...
assert.match(component, /currentCoverPresentationMode\.value === COVER_PRESENTATION_MODES\.MIXED[\s\S]{0,80}\? "cover"[\s\S]{0,40}: "contain"/);
...
assert.match(component, /fillScale = Math\.max\([\s\S]{0,180}mediaWidth \/ imageWidth,[\s\S]{0,100}mediaHeight \/ imageHeight/);
assert.match(component, /Math\.max\(fillScale \* COVER_HOVER_BOOST, COVER_HOVER_MIN_SCALE\)/);
assert.match(component, /hoverShiftX = Math\.max\(0, \(mediaWidth - imageWidth\) \/ 2\)/);
assert.match(component, /transform-origin: right center/);
assert.match(component, /@media \(hover: hover\) and \(pointer: fine\)[\s\S]{0,520}category-stream__preview-image\.n-image img[\s\S]{0,180}left: var\(--cover-hover-shift-x, 0px\);[\s\S]{0,180}transform: scale\(var\(--cover-hover-scale, 1\.35\)\)/);
