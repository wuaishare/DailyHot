...
assert.match(template, /<n-image[\s\S]{0,420}:preview-src="coverSrc\(entry\.cover\)"[\s\S]{0,320}object-fit="contain"[\s\S]{0,260}onLoad: handleCoverImageLoad/);
...
assert.match(component, /@media \(hover: hover\) and \(pointer: fine\)[\s\S]{0,520}category-stream__preview-image\.n-image img[\s\S]{0,180}left: var\(--cover-hover-shift-x, 0px\);[\s\S]{0,180}transform: scale\(var\(--cover-hover-scale, 1\.1\)\)/);
...
assert.match(component, /@media \(max-width: 680px\)[\s\S]*category-stream__preview-image\.n-image img[\s\S]{0,100}max-width: 76px;[\s\S]{0,80}max-height: 64px/);
